...
class Box {
  protected:
    ...
    void (*event_function_pointer)() = nullptr;

    void event_function() {
        if (event_function_pointer) {
            event_function_pointer();
        }
    }
  ...
  public:
     ...
     */
    enum class TYPE { BAR, SLIDER, LABEL, TEXT};
     ...
     */
    Box(const char *title, bool isEditable, void (*event_function)(), uint8_t height);
     ...
     * @return 
     */
    virtual void calculate_width() = 0;
     ...
     * @return
     */
    virtual void keyboard_event(const uint8_t *buff, uint8_t numOfKeys) = 0;
     ...
     * @return
     */  
    void set_unactive(bool delPrt = false);
